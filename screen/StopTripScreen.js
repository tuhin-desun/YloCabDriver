...
import {compeleteTrip, getDriverProfile} from "../services/APIServices";
...
export default function StopTripScreen(props) {
...
const cancelReasons = [{
    ...
    useEffect(()=>{
        getDriverProfile(context.driverData.id).then((response) => {
            if (response.check == 'success') {
                context.setDriverData(response.data);
            }
    ...
    const endBooking = () => {
        ...
        //PAYMENT_PENDING MEANS PAYMENT IS PENDING AND TRIP IS FINISHED
        booking.status = 'PAYMENT_PENDING';
        let driverId = context.driverData.id;
        compeleteTrip(booking)
            .then((response)=>{
    ...
    const renderButtons = () => {
        let btnText = "Compelete Job";
        let functionRef = endBooking;
        if(curBooking.tripdata.type == "round"){
            let today = moment(new Date());
            let endDate = moment(new Date(curBooking.tripdata.returnDateStamp));
            if(endDate.diff(today, 'minutes') > 0){
                btnText = `You can not complete trip before ${curBooking.tripdata.returnDate}`;
                functionRef = () => {errorToast("Error","YOU CAN NOT COMPLETE JOB")}
            }
        }
                    ...
                    <View style={styles.cancelModalInnerContainer}>
                        ...
                        <View style={styles.cancelContainer}>
                            <View style={styles.cancelReasonContainer}>
                                <Text style={styles.cancelReasonText}>{"Cancel Reason Modal Title"}</Text>
                            </View>
                            ...
                            <View style={styles.cancelModalButtosContainer}>
                                ...
                                <Button
                                    title={"No driver found ok button"}
                                    titleStyle={styles.signInTextStyle}
                                    onPress={() => {
        ...
        <Root colors={[{ card: colors.black, label: colors.white }]} >
            ...
            <View style={styles.mapcontainer}>
                {curBooking ?
                    ...
                    : null}
                    <TouchableOpacity
                        style={[styles.floatButton, { bottom: 50 }]}
