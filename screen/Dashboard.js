...
export default class Dashboard extends React.Component {
  ...
  componentDidMount = () => {
    ...
    this.focusListeners = this.props.navigation.addListener("focus", () => {
      this.getSettings();
      getDriverProfile(this.context.driverData.id)
        .then((response) => {
    ...
    this.tempBookingRef = firebase
      .database()
      .ref("TempBooking/" + this.context.driverData.id);
  ...
  componentWillUnmount() {
    ...
    if (this.state.tempWatcher !== null) {
      this.state.tempWatcher?.remove();
    }
  }
    ...
    fetchSettings()
      .then((response) => {
        if (response.type == "success") {
          ...
          this.setState({
            providerTimeout: getValueOfSetting(
              response.data,
    ...
    if (bookData != "null") {
      ...
      if (bookData.hasOwnProperty("bookingID")) {
        ...
        if (bookData.status == "SEARCHING") {
          this.playSound();
          this.props.navigation.navigate("ActiveBooking", {
            bookData: bookDataArr,
            timerState: true,
            providerTimeout: this.state.providerTimeout,
          });
        }

        if (bookData.status == "ACCEPTED") {
          this.props.navigation.navigate("ReachedScreen", {
            curBooking: bookDataArr,
          });
        }
          ...
          <View style={{ flex: 1, backgroundColor: Colors.white }}>
            ...
            >
              ...
              <Text style={{ fontSize: 18 }}>
                Customers are waiting to travel with you
              </Text>
            </View>
            <View>
              <Image
