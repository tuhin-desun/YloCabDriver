...
export default function DriverTrips(props) {
  ...
  const [isTimerActive, setIsTimerActive] = useState(
    props.route.params.timerState ?? true
  );
  const [providerTimeout, setProviderTimeout] = useState(
    props.route.params.providerTimeout ?? 30
  );
  ...
  useEffect(() => {
    ...
    let point1 = {
      lat: bookData.tripdata.pickup.lat,
      lng: bookData.tripdata.pickup.lng,
    };
    let point2 = {
      lat: bookData.tripdata.drop.lat,
      lng: bookData.tripdata.drop.lng,
    };
        ...
        >
          <MapView
            ...
            initialRegion={{
              ...
              latitudeDelta:
                activeBookings && activeBookings.length >= 1 ? 0.0922 : 0.0822,
              longitudeDelta:
                activeBookings && activeBookings.length >= 1 ? 0.0421 : 0.0321,
            }}
          >
            <Marker
              coordinate={{
            ...
            <Marker
              coordinate={{
                latitude: bookData.tripdata.drop.lat,
                longitude: bookData.tripdata.drop.lng,
              }}
        ...
        <View style={[styles.mapDetails, { paddingHorizontal: 10 }]}>
          <View style={styles.dateView}>
            <Text style={styles.listDate}>
              {bookData.tripdata.type == "round"
                ? bookData.tripdata.tripDate +
                  " to " +
                  bookData.tripdata.returnDate
                : bookData.tripdata.tripDate}
            </Text>
            <Text style={styles.listDate}>
              {bookData.tripdata.type == "round"
                ? "Round Trip"
                : bookData.estimate.estimateDistance > 50
                ? "Out Station"
                : null}
            </Text>
          </View>
          ...
          <View style={styles.distance}>
            <View style={styles.distanceColumn1}>
              ...
              <Text style={styles.caption}>
                {bookData.estimate.estimateTime
                  ? parseFloat(bookData.estimate.estimateTime).toFixed(1) > 59
                    ? `${parseFloat(
                        bookData.estimate.estimateTime / 60
                      ).toFixed(0)} hour`
                    : `${parseFloat(bookData.estimate.estimateTime).toFixed(
                        0
                      )} minutes`
                  : `${0} minutes`}
              </Text>
            ...
            <View style={styles.distanceColumn2}>
              ...
              <Text style={styles.caption}>
                {bookData.estimate.estimateDistance
                  ? parseFloat(bookData.estimate.estimateDistance).toFixed(2)
                  : 0}{" "}
                {"KM"}
              </Text>
            </View>
          ...
          <View style={styles.addressViewStyle}>
            <View style={{ flexDirection: "row", alignItems: "center" }}>
              ...
              <Text style={styles.addressViewTextStyle}>
                {bookData.tripdata.pickup.add}
              </Text>
            </View>
            <View style={styles.fixAdressStyle}>
              <View style={styles.redDot}></View>
              <Text style={styles.addressViewTextStyle}>
                {bookData.tripdata.drop.add}
              </Text>
            </View>
          </View>
          <View
          ...
          >
            ...
            <Text style={styles.rateViewTextStyle}>
              {"₹ "}
              {bookData
                ? bookData.estimate.estimateFare > 0
                  ? parseFloat(bookData.estimate.estimateFare).toFixed(2)
                  : 0
                : null}
            </Text>
          </View>
