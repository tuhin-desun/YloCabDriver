...
import { updateTempBookingDataFirebase, getDriverUpdatedLocation, getTravelDistanceandTime } from "../utils/helper";
import { reachedLocation, cancelTrip } from "../services/APIServices";
...
export default function ReachedScreen(props) {
    ...
    // console.log("Context", context.locationUpdateData)
    // getDriverUpdatedLocation(context.driverData.id);
    const bookingId = props.route.params.curBooking[0].bookingId;
    ...
    const [curBooking, setCurBooking] = useState(props.route.params.curBooking[0]);
    ...
    useEffect(() => {
        ...
        let point1 = { lat: context.locationUpdateData.location.latitude, lng: context.locationUpdateData.location.longitude };
    ...
    useEffect(() => {
        ...
        // console.log(result)
    }, [context.locationUpdateData.location.latitude])

    useEffect(() => {
        // alert(JSON.stringify(context.locationUpdateData.location.latitude))
    }, [context.locationUpdateData])
    ...
    const startBooking = () => {
        ...
        let reqObj = {
            booking : booking,
            lat: context.locationUpdateData.location.latitude, 
            lng: context.locationUpdateData.location.longitude
        }
        reachedLocation(reqObj)
            .then((response) => {
                setReachedLoading(false);
                // console.log(response);return;
                if (response.type == "success") {
                    updateTempBookingDataFirebase(booking, driverId);
                } else {
                    alert(response.msg);
                }
            })
            .catch((err) => { alert("Something wrong try later!!"); })
    }
    ...
    const startNavigation = () => {
        ...
        try {
            ...
            data = {
                source: {
                    latitude: context.locationUpdateData.location.latitude,
                    longitude: context.locationUpdateData.location.longitude
                },
                destination: {
        ...
        <View style={styles.mainContainer}>
                ...
                <View style={styles.topRightContainer}>
                    <TouchableOpacity style={styles.whereButton}>
                        <View style={styles.whereContainer}>
                            <Text numberOfLines={1} style={styles.whereText}>{`Distance: ${reachDistance}`}</Text>
                            <Icon
                                name='gps-fixed'
                                color={colors.WHITE}
                                size={23}
                    ...
                    <TouchableOpacity style={styles.dropButton}>
                        <View style={styles.whereContainer}>
                            <Text numberOfLines={1} style={styles.whereText}>{`Time: ${reachTime}`}</Text>
                            <Icon
                                name='search'
                                type='feather'
                                color={colors.WHITE}
            ...
            <View style={styles.mapcontainer}>
                ...
                <TouchableOpacity
                    ...
                    onPress={() => onPressCall(curBooking.user_mobile)}
                >
                    <Icon
                        name="ios-call"
                        type="ionicon"
                    ...
                    {curBooking && curBooking.status != "SEARCHING" ?
                        <View style={styles.driverDetails}>
                            <View style={styles.driverPhotoContainer}>
                                <Image source={curBooking.user_avatar ? { uri: curBooking.user_avatar } : require('../assets/profilePic.png')} style={styles.driverPhoto} />
                            </View>
                            <View style={styles.driverNameContainer}>
                                <Text style={styles.driverNameText}>{curBooking.user_name}</Text>
                            </View>
                        </View>
                        : null}
                </View>
